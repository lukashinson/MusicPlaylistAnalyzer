...
namespace MusicPlaylistAnalyzer
{
    public class musicReport
    {
        public static string GenerateText(List<musicCategories> musicCategoriesList)
        {
            ...
            var alternativeSongs = from musicCategories in musicCategoriesList where musicCategories.Genre == "Alternative" select musicCategories;
            var AltSongs = 0;
            foreach (var song in alternativeSongs)
            {
                AltSongs += 1;
            }
            report += AltSongs;
